...
export function Bonuses() {
  ...
  return (
    <section className="w-full py-12 md:py-24 lg:py-32 bg-[#f9bf81] text-black">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          ...
          <h2 className="text-3xl font-bold tracking-tighter sm:text-5xl uppercase text-card-foreground">E ainda tem mais...</h2>
          ...
          {bonusItems.map((bonus, index) => (
            <div key={index} className="flex flex-col items-center text-center gap-4">
               ...
               <div className="space-y-2">
                <h3 className="text-2xl font-bold flex items-center justify-center gap-2 text-card-foreground">
                  <Gift className="w-7 h-7 text-primary" /> {bonus.title}
                </h3>
                <p className="text-lg text-card-foreground/90">{bonus.description}</p>
               </div>
            </div>
          ))}
        </div>
